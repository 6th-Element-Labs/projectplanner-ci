...
- [x] **Phase 5.5 — Live Inbox (#9)** — the Gmail-poll source onto the Phase-5 core. `gmail_source.py` (IMAP app-password, sender allow-list, **no-op until `PM_IMAP_*` set**) → `inbox.process` (dedupe → ingest+triage → **queue**) → `store.inbox`. Review **Inbox tab** (pending badge, per-row drop, Confirm selected / Dismiss; confirm applies as "Maxwell (email)") + a **Simulate email** path to test without a mailbox. `projectplanner-inbox.timer` (every 5 min, left disabled until creds). API: `/api/inbox`, `/api/inbox/{id}/{confirm,dismiss}`, `/api/inbox/simulate`, `/api/inbox/poll`. Go live = `PM_IMAP_*` in `.env` + enable the timer.
