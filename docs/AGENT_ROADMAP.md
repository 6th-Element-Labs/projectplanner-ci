...
- [x] **Phase 5 — ingest + triage core** (the unified pipeline behind #5 and #9). Incremental/persistent RAG (`rag.add_document` + `store.rag_docs`, cross-process fresh) — artifacts citable at runtime. `agent.triage` + `_system_triage` (ground / verify exit-criteria / blast-radius / decide, no keyword-matching) + `propose_new_task`. `intake.ingest_and_triage` → `POST /api/intake`; MCP `ingest_and_triage`; **Intake box** in the Ask tab (paste/upload → summary + Confirm-all updates + Create-all new tasks). Verified: a transcript produced a completion (w/ exit-criteria caution), a slip + cascade rebaseline, and a new task — all propose-to-confirm.
- [x] **Phase 5.5 — Live Inbox (#9), AUTONOMOUS** — IMAP source on the Phase-5 core. `inbox_source.py` (IMAP app-password, no-op until `PM_IMAP_*`) delegates source-independent routing to `switchboard.integrations.inbox_routing`, then calls `inbox.process`: dedupe → ingest+triage → in autonomous mode (default, `PM_INBOX_AUTONOMOUS=1`) **auto-applies** the implied changes (audited "Maxwell (email)") AND **emails the sender back** (`notify.reply`) — answering questions and confirming changes in past tense; conservative (no speculative cascades). Inbox tab is a **log** of what it did (queue/confirm kept as the `=0` fallback). Verified via Simulate: an update auto-moved FMP-11 → In Progress; a question got a no-change answer reply. `projectplanner-inbox.timer` (5 min). **Go live: enable 2FA on plan@taikunai.com → make a Gmail App Password → set `PM_IMAP_PASSWORD`/`PM_SMTP_PASSWORD` → `systemctl enable --now projectplanner-inbox.timer`.**
