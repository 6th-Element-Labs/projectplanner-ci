...
**Ingest charter:** [ADR-0016](decisions/0016-ingest-inbox-process-strangler.md)  
**Board:** `project=switchboard` · workstream **ARCH-MS**  
**Deliverables:** `arch-ms-phase-0` · `arch-ms-phase-1` (modular monolith exit) · `arch-ms-phase-2` · `arch-ms-phase-3` · `arch-ms-tasks-live-cut` (segmentation — **Tasks chart green** via ARCH-MS-100) · `arch-ms-coord-service` (Coord/board peel) · `arch-ms-deliverables-service` (Deliverables/mission peel) · `arch-ms-tally-service` (Tally/economics peel) · `arch-ms-ingest-service` (Ingest/inbox peel)
...
**Ingest service view:** [`?project=switchboard&deliverable=arch-ms-ingest-service#tab-mission`](https://plan.taikunai.com/?project=switchboard&deliverable=arch-ms-ingest-service#tab-mission)

---

## Ingest / inbox process strangler (service #6 candidate) — Mode A

**Charter:** [ADR-0016 — Ingest/inbox process strangler](decisions/0016-ingest-inbox-process-strangler.md)  
**Deliverable:** `arch-ms-ingest-service` · mission `arch-ms-segmentation`  
**Depends on:** Tally charter (ARCH-MS-98)  
**Thin surface:** [`docs/ingest/thin_day_one_surface.md`](ingest/thin_day_one_surface.md) — port **`:8126`**

| Milestone | Intent |
|---|---|
| Independence | Charter + ports/writers/ops + Go/No-Go before any cut |
| Process cut | Conditional Ingest uvicorn + Caddy (**Go only**) |
...
| **ARCH-MS-119** | Ingest independence: writers, idempotency, routing, and capacity | 🔴 No-Go | Executable route/writer/retry/isolation/auth/contention/capacity gate; `docs/ingest/ingest_independence_verdict.json`; `scripts/arch_ms119_ingest_independence.py`; ARCH-MS-120 fails closed; keep in-process |
...
| **ARCH-MS-104** | Coord independence: routes, writers, auth, SQLite, capacity | 🟢 | Go artifact + executable port/import gate; `docs/coord/coord_independence_verdict.json` |
| **ARCH-MS-105** | Standalone Coord service + side-by-side parity on `:8123` | 🟢 | PR #568 — `services/coord/{app,settings,health}`; parity/boundary proof |
| **ARCH-MS-106** | Coord production unit, exact Caddy cut, dual-strip, rollback gates | 🟡 | `switchboard-coord.service`; exact `:8123` handles; fail-closed deploy proof |
