...
- **Date:** 2026-07-13
- **Scope:** Claude Code, Codex CLI/app, Cursor, OpenCode CLI, LangGraph, OpenAI and Anthropic
...
| `opencode-cli` | Yes: launch a new supervised `opencode --auto` process with session-scoped Switchboard MCP. | Vendor session resume exists; Switchboard has not wired a resume launcher yet. Missing handle fails closed. | Persist the OpenCode session id and host/cwd only after a live resume proof. [OpenCode CLI](https://opencode.ai/docs/cli/) |
