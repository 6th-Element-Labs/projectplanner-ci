...
| Claude subscription OAuth / `setup-token` (portable) | `vendor_confirmation_required` | None through Switchboard | Disabled | Anthropic documents `setup-token`, but its legal guidance says third-party products must not offer Claude.ai login or route Free/Pro/Max credentials on users' behalf. A long-lived token is not permission to broker it. Stays disabled. |
| Claude subscription login (host-bound) | `supported_host_bound` | Exact registered, user-owned persistent Agent Host only | Operator runs `claude /login` on that host; credential stays in the host OS keychain, never minted (`setup-token`), exported, or brokered | CO-22 operator decision (`decision-979`, 2026-07-24). Mirrors the Cursor host-bound posture: the CLI authenticates itself as it would for a human at that keyboard, so nothing is *routed* third-party. Operator-accepted residual Consumer-Terms risk; no separate written Anthropic confirmation. Exclusive, `max_parallel=1`. |
