...
Coverage must also contain both the unchanged `passthrough` baseline and shadow-only
`scan` mode. Either mode alone persists a `required_mode_missing:<mode>` blocker and
cannot advance or gain named-mechanism authority.
