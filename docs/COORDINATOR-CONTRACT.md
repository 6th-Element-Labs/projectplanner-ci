...
- **May:** create project-scoped wake intents (`dispatch_to_claude_code` / `dispatch.dispatch` → `request_wake(policy={mode:"claim_next"})`); post advisory coordination comments/heads-ups; write its own `set_agent_state`; select model tier / lane hints per PRD §20 (model right-sizing) as *dispatch metadata*.
...
- **Mechanism (existing rails):** `dispatch_to_claude_code(task_id, project)` → `dispatch.dispatch(task_id, actor, project)` → `store.request_wake(selector={runtime, lane, agent_id}, policy={mode:"claim_next"}, task_id, idem_key)`. The wake is a **durable, idempotent intent**, not a push. An **eligible Agent Host in that environment** `claim_wake`s it and starts/reuses a runtime session that pulls the task via `claim_next`. See [AGENT-HOST-SPEC](AGENT-HOST-SPEC.md).
