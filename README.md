...
Runs as a small Caddy-fronted control plane on one VM. The compatibility web app remains on
`:8110`, MCP is on `:8111`, and selected Auth, Tasks, Coordination, Deliverables, and Ingest
routes have been cut to bounded-context processes on `:8121`–`:8126`. A bundled LiteLLM
gateway runs on `:8095`.
...
See [deploy/PROVISION.md](deploy/PROVISION.md). The deployment uses one Python environment,
Caddy, and systemd units for the web/MCP surfaces, bounded-context services, and background
jobs.

## Docs

Start with [`docs/INDEX.md`](docs/INDEX.md). Contributors and coding agents should also read
[`AGENTS.md`](AGENTS.md).

The cornerstone lifecycle material is the
