...
ok(report.get("verdict") == "go", "remediated verdict is explicit Go")
ok(report.get("process_build_authorized") is True, "Go authorizes standalone Ingest build")
...
ok(report.get("resource_headroom_bytes") == 1159168, "64 MiB cap preserves resident reserve")
...
ok(gate.go_only_task_authorized(verdict, "ARCH-MS-120") is True,
   "machine verdict authorizes Go-only ARCH-MS-120")
...
ok(go_attempt.returncode == 0, "Go-only CLI gate authorizes ARCH-MS-120")
