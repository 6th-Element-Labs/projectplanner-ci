...
from switchboard.domain.completion import state_machine as sm
...
class FindingShapeIsFlatTest(unittest.TestCase):
    ...
    def test_the_constructor_documents_the_trap(self):
        ...
        self.assertIn("_finding_detail", doc)
...
class FindingDetailAccessorTest(unittest.TestCase):
    def test_it_reads_the_flat_shape_the_gate_emits(self):
        finding = _finding("pr_not_mergeable", "m", mergeable=False,
                           merge_state="dirty")
        self.assertIs(sm._finding_detail(finding, "mergeable"), False)
        self.assertEqual(sm._finding_detail(finding, "merge_state"), "dirty")
    ...
    def test_it_still_reads_a_nested_finding(self):
        ...
        self.assertIs(sm._finding_detail(nested, "mergeable"), False)
        self.assertEqual(sm._finding_detail(nested, "merge_state"), "dirty")

    def test_it_tries_each_alias_in_order(self):
        finding = _finding("pr_not_mergeable", "m", mergeStateStatus="DIRTY")
        self.assertEqual(
            sm._finding_detail(finding, "merge_state", "mergeStateStatus"), "DIRTY")

    def test_a_missing_detail_is_none_not_an_error(self):
        self.assertIsNone(sm._finding_detail({}, "mergeable"))
        self.assertIsNone(sm._finding_detail({"details": "not-a-mapping"}, "mergeable"))
...
class ConsumersReadRealFindingsTest(unittest.TestCase):
    ...
    def test_the_conflict_decomposer_fires_on_a_finding_only_conflict(self):
        ...
        finding = _finding("pr_not_mergeable",
                           ...
                           mergeable=False, merge_state="dirty")
        decision = sm._merge_conflict_decision({}, [finding])
        self.assertIsNotNone(
            decision, "a conflicted PR must reach pr_merge_conflict, not read as missing")
    ...
    def test_the_artifact_lift_fires_on_a_real_evidence_finding(self):
        ...
        self.assertEqual(
            sm._missing_artifact_identity([finding]),
            {"missing_artifact": {"expected_key": "executed_test_run"}})

    def test_a_clean_pr_produces_no_conflict_decision(self):
        self.assertIsNone(sm._merge_conflict_decision({"mergeable": True}, []))
        self.assertIsNone(sm._merge_conflict_decision({}, []))
...
_DETAILS_READERS_ALLOWED = {"_finding_detail"}
...
class NobodyReadsFindingDetailsByHandTest(unittest.TestCase):
    def test_the_completion_domain_goes_through_the_accessor(self):
        ...
        """
        offenders: list[str] = []
        for path in sorted(_COMPLETION_PACKAGE.glob("*.py")):
            offenders.extend(_direct_details_reads(path))
        self.assertEqual(
            offenders, [],
            "read merge-gate finding details via state_machine._finding_detail(); "
