...
ok(report.get("failed_gates") == [], "all five independence gates pass")
checks = report.get("checks") or {}
ok(checks.get("router_inventory_complete") is True, "all 27 router routes are inventoried")
ok(checks.get("day_one_surface_exact") is True, "exact seven-route day-one surface is locked")
...
ok(checks.get("writer_transactions_complete") is True,
   "all twenty monolith-only routes have explicit transaction/orchestration boundaries")
...
ok(probe.get("ok") is True, "concurrent snapshot probe passes")
ok(probe.get("lock_errors") == 0, "concurrent probe has zero lock errors")
ok(probe.get("snapshot_mismatches") == 0, "readers never observe a split revision")

verdict = gate.load_verdict(ROOT / "docs" / "deliverables" / "deliverables_independence_verdict.json")
ok(len(verdict.get("writer_inventory") or []) == 20,
   "all twenty non-service routes explicitly stay monolith-owned")
