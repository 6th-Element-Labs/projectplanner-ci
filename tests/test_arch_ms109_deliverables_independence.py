...
ok(checks.get("router_inventory_complete") is True, "all 29 router routes are inventoried")
...
ok(checks.get("writer_transactions_complete") is True,
   "all twenty-one monolith-only routes have explicit transaction/orchestration boundaries")
...
ok(len(verdict.get("writer_inventory") or []) == 21,
   "all twenty-one non-service routes explicitly stay monolith-owned")
