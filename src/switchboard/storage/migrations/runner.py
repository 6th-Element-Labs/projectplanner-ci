...
ADDITIVE_COLUMN_MIGRATIONS: List[Tuple[str, str, str, str]] = [
    ...
    ("0100_external_ci_runs_execution_owner", "external_ci_runs", "execution_owner_id",
     "ALTER TABLE external_ci_runs ADD COLUMN execution_owner_id TEXT"),
    ("0101_external_ci_runs_execution_fence", "external_ci_runs", "execution_fence",
     "ALTER TABLE external_ci_runs ADD COLUMN execution_fence INTEGER NOT NULL DEFAULT 0"),
    ("0102_external_ci_runs_execution_lease", "external_ci_runs", "execution_lease_expires_at",
     "ALTER TABLE external_ci_runs ADD COLUMN execution_lease_expires_at REAL"),
    ("0103_external_ci_runs_execution_started", "external_ci_runs", "execution_started_at",
     "ALTER TABLE external_ci_runs ADD COLUMN execution_started_at REAL"),
...
DDL_MIGRATIONS: List[Tuple[str, str]] = [
    ...
    # COORD-112 restores only COORD-109's dormant persistence contract.
    # These rows are evidence/history, never liveness, scope, or v1 completion
    # authority.  Keep the original immutable migration identities because
    # SIMPLIFY-31 removed source definitions without dropping retained tables.
    ("0123_mission_items",
