...
This module deliberately accepts already-hydrated authority facts.  It does not
read Work Sessions as liveness, infer Human from board status, or call a start
port.  The scoped worker may execute the returned ``start_task`` action later.
"""
...
def decide_mission_transition(context: Mapping[str, Any]) -> dict[str, Any]:
    ...
    Precedence mirrors ``docs/MISSION-BOT-V4.md``.  ``board_status`` is
    intentionally ignored: board Blocked is neither Human authority nor
    Capacity liveness.
    """
