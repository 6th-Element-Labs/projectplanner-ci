...
def create_router(*, resolve_project: ProjectResolver,
    ...
    When ``sibling_bc_only`` is True (ARCH-MS-92 dual-strip), mount only those
    sibling BC subpaths so Mode A CRUD stays exclusive to ``:8122``.
    """
    if thin_mode_a and sibling_bc_only:
        raise ValueError("thin_mode_a and sibling_bc_only are mutually exclusive")
    router = APIRouter()
