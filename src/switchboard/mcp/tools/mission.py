"""Thin MCP adapters for the Mission Bot v4 agent protocol."""
...
def get_mission_context(task_id: str, project: str = "maxwell") -> str:
    """Return bounded current authority facts plus recent mission history."""
    return _services().dumps(queries.get(task_id, project=project))


def list_mission_history(
    task_id: str, after_sequence: int = 0, limit: int = 50,
    project: str = "maxwell",
) -> str:
    """Return a cursor-paginated page of the append-only mission journal."""
...
def yield_mission(
    task_id: str, execution_id: str, generation: int, observed_through: int,
    outcome: str, requested_role: str, ctx: Context, head_sha: str = "",
    project: str = "maxwell",
) -> str:
    """Yield one authenticated current execution for wait or a fresh role."""
    services = _services()
    ...
    agent_hint = str(
        ...
        or (
            ...
            if str(principal.get("kind") or "").lower() in {"agent", "direct_session"}
            else ""
        )
        or ""
    )
    ...
    try:
        result = commands.yield_mission(
            task_id, project=project, execution_id=execution_id,
            generation=generation, observed_through=observed_through,
            outcome=outcome, requested_role=requested_role,
            actor=str(binding.get("agent_id") or binding.get("actor") or ""),
            head_sha=head_sha,
        )
    except commands.MissionJournalError as exc:
        return services.dumps({
            "accepted": False, "error": exc.code, "message": str(exc),
            "task_id": task_id,
        })
    return services.dumps({"accepted": True, **result})


MISSION_TOOL_NAMES = ("get_mission_context", "list_mission_history", "yield_mission")


def register_mission_tools(
    mcp: Any, services: MissionToolServices,
