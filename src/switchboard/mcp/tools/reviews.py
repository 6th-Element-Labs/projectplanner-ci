...
def record_review_verdict(verdict_json: str, ctx: Context,
    ...
    """Persist an authenticated verdict for the task's exact current PR head.
    ...
    Identical writes replay idempotently. A later changes_requested write may only
    append blocking findings; it creates an immutable superseding revision and can
    never turn unresolved same-head defects back into a pass.
    """
    services = _services()
