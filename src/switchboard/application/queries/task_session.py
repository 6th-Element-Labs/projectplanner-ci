...
def _deliverable(task_id: str, project: str) -> Optional[dict[str, Any]]:
    ...
    if not deliverable_id:
        return link
    return store.get_deliverable(deliverable_id, project=project) or link
...
def execute_for(task_id: str, *, project: str) -> Optional[dict[str, Any]]:
    ...
    if not task:
        return None
    sessions = store.list_runner_sessions(
    ...
    wakes = store.list_wake_intents(task_id=task_id, project=project, limit=100)
