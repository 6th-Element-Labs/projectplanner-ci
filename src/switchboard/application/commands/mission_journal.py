...
def yield_mission(
    task_id: str, *, project: str, execution_id: str, generation: int,
    observed_through: int, outcome: str, requested_role: str, actor: str,
    head_sha: str = "",
    repository: MissionJournalRepository = default_mission_journal_repository,
) -> dict[str, Any]:
    result = repository.yield_execution(
        task_id, project=project, execution_id=execution_id, generation=generation,
        observed_through=observed_through, outcome=outcome,
        requested_role=requested_role, actor=actor, head_sha=head_sha,
    )
    identity = dict(result.pop("execution_identity"))
    if not result.get("created"):
        result["surrender_requested"] = True
        result["surrender"] = {"updated": False, "idempotent_replay": True}
        return result
