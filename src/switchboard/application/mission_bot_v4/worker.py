"""The fenced W2 Mission Bot v4 pager.
...
def _wait(reason: str, *, task_id: str) -> dict[str, Any]:
    return {
...
def _start_was_admitted(receipt: Mapping[str, Any]) -> bool:
    ...
    return str(receipt.get("action") or "") in {
        "started", "starting", "attached",
    }


def tick_scoped_mission(
    task_id: str, *, project: str, scope_authority: Mapping[str, Any],
    actor: str, ports: ScopedMissionWorkerPorts,
) -> dict[str, Any]:
    ...
    if not authority.get("allowed"):
        return _wait(
            str(authority.get("error") or "scope_inactive"), task_id=task_id,
        )
    ...
    # takeover, or another worker tick therefore fails closed.
    ...
    if not boundary.get("allowed"):
        return _wait(
            str(boundary.get("error") or "scope_inactive"), task_id=task_id,
        )
    ...
    receipt = ports.start_task(
        ...
        role=role,
        source_sha=str(
            event.get("head_sha") or (task.get("git_state") or {}).get("head_sha") or ""
        ),
