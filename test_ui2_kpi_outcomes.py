...
try:
    ...
    # Project isolation: the switchboard KPI must NOT leak into the default board.
    store.init_db("maxwell")
    r = client.get("/tally/v1/kpis", params={"project": "maxwell"})
    ok(all(k["id"] != kpi_id for k in r.json().get("kpis", [])),
       "KPI list is project-scoped (switchboard KPI absent from maxwell)")
    ...
    # Scoping guard: tally reads/writes must carry the project explicitly (the
    # global fetch wrapper only scopes `api/…` URLs, not `tally/…`).
    scope_needles = ["tally/v1/kpis?project=", "tally/v1/outcomes?limit=100&project=",
                     "project: this._pmProject()"]
    missing_scope = [n for n in scope_needles if n not in appjs]
    ok(not missing_scope, f"tally calls are project-scoped in app.js (missing: {missing_scope})")
    ok("fetch('tally/v1/kpis')" not in appjs and 'fetch("tally/v1/kpis")' not in appjs,
       "no unscoped bare tally/v1/kpis fetch remains")
