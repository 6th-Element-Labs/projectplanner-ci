...
def _deliverable_projection(project: str, deliverable_id: str) -> Optional[Dict[str, Any]]:
    ...
    for tp, rs in by_proj.items():
        ...
        if store.has_project(tp):
            ...
            with _conn(tp) as tc:
                # Chunk the status lookup to stay under SQLite's bound-variable limit, matching
                # store._provenance_by_task's own chunking (a deliverable could link many tasks).
                for i in range(0, len(tids), 400):
                    batch = tids[i:i + 400]
                    ph = ",".join("?" * len(batch))
                    for row in tc.execute(
                        f"SELECT task_id, status FROM tasks WHERE task_id IN ({ph})", batch):
