...
def _pr_number_from_evidence(evidence):
...
def _pr_repo_from_evidence(evidence, repo=None):
    if repo:
...
def ensure_pr_ready(evidence, *, cwd=None, repo=None, token=None):
    """Mark the worker's PR non-draft before merge authorization (ADAPTER-30).

    Idempotent: an already-ready PR is a no-op. Missing PR evidence is skipped so
    offline/docs completions are unchanged. Shared across every runtime that exits
    through complete_claim / run_session.
    """
...
def complete_claim(project, claim_id, evidence, base=None, token=None, final_status="",
    ...
    data = _evidence_mapping(evidence)
    pr_ready = ensure_pr_ready(data, token=token)
    ...
    if data or pr_ready.get("status") != "skipped_no_pr":
        data["pr_ready"] = pr_ready
        ev = json.dumps(data)
