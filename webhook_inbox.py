...
def _apply_row(row: Mapping[str, Any], project: str) -> Dict[str, Any]:
    ...
    projection = github_mission_events.project_delivery(
        event, payload, project=project,
    )
    if projection.get("events"):
        combined = {**result, "mission_events": projection}
        if result.get("action") == "ignored":
            combined["base_action"] = "ignored"
            combined["action"] = projection["action"]
        return combined
    if projection.get("action") != "ignored":
