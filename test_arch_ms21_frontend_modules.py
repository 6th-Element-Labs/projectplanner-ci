...
modules = ("api", "state", "board", "mission")
positions = [INDEX.find(f'src="js/{name}.js?v=') for name in modules]
app_position = INDEX.find('src="app.js?v=')
ok(all(pos >= 0 for pos in positions), "index loads all four frontend modules")
...
for namespace in ("SwitchboardState", "SwitchboardBoard", "SwitchboardMission"):
