...
# The dock is 380px wide and check names come from GitHub, so an untruncated label used to clip
# the chip mid-word and wrap the PR number onto a second line.
long_name = "Switchboard CI / VM gate (ubuntu-latest, py3.12)"
wide = render([pr(ci_state="failure", ci_failing=[long_name], mergeable_state="dirty")])[0]["html"]
ok("VM gate (ubuntu-latest, py3.12) failed" in wide,
