...
def all_rag_chunks(project: str = DEFAULT_PROJECT) -> List[Dict[str, Any]]:
    with _conn(project) as c:
...
def rag_docs_max_id(project: str = DEFAULT_PROJECT) -> int:
    with _conn(project) as c:
        return c.execute("SELECT COALESCE(MAX(id), 0) FROM rag_docs").fetchone()[0]


def all_rag_rows(project: str = DEFAULT_PROJECT) -> List[Dict[str, Any]]:
...
def update_rag_embedding(rag_id: int, embedding: List[float], project: str = DEFAULT_PROJECT):
    with _conn(project) as c:
