...
const TeepPlan = {
    ...
    async loadKpisAndOutcomes() {
        try {
            const [kres, ores] = await Promise.all([
                fetch('tally/v1/kpis', { cache: 'no-store' }),
                fetch('tally/v1/outcomes?limit=100', { cache: 'no-store' }),
            ]);
    ...
    async submitKpi() {
        ...
        if (!name) { this._dlFlash('dl-kpi-flash', 'Name is required.', 'text-danger'); return; }
        const num = (id) => { const v = document.getElementById(id).value; return v === '' ? undefined : Number(v); };
        const body = {
            name, unit: (document.getElementById('dl-kpi-unit').value || '').trim(),
    ...
    async updateKpiValue(kpiId) {
        ...
        try {
            await this._dlSend(`tally/v1/kpis/${encodeURIComponent(id)}`, 'PATCH', { current_value: val });
            await this.refreshMissionPage();
    ...
    async verifyTallyOutcome(outcomeId) {
        ...
        try {
            await this._dlSend(`tally/v1/outcomes/${encodeURIComponent(id)}/verify`, 'POST', {});
            await this.refreshMissionPage();
    ...
    async rejectTallyOutcome(outcomeId) {
        ...
        if (reason === null) return;
        try {
            await this._dlSend(`tally/v1/outcomes/${encodeURIComponent(id)}/reject`, 'POST', { reason: reason || 'rejected' });
            await this.refreshMissionPage();
