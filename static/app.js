...
const TeepPlan = {
    ...
    _prConditions(x) {
        const out = [];
        const failing = (x.ci_failing || [])[0];
        if (x.ci_state === 'failure') out.push({ key: 'ci_failed', label: failing ? `${failing} failed` : 'CI failed', tone: 'red', icon: 'x' });
    ...
    _dockPrHtml(x) {
        ...
        return `<div class="p-2 border rounded mb-2" style="border-left:2px solid ${accent} !important;border-top-left-radius:0;border-bottom-left-radius:0;">
            <div class="d-flex align-items-center gap-2">
                ${this._dockBadge(primary.label, primary.tone, primary.icon)}
                ${secondary ? `<span class="badge bg-transparent border text-secondary">+ ${this.esc(secondary.label)}</span>` : ''}
                <span class="ms-auto text-secondary font-monospace" style="font-size:12px;">#${this.esc(String(x.number))} · ${this.esc(this._fleetAge(x.updated_at))}</span>
            </div>
    ...
    async _openDirectRunnerWhenReady(taskId) {
        ...
        const el = document.getElementById('edit-flash-dev');
        if (el) {
            el.textContent = 'The Mac has not published the CLI yet. The assignment remains queued; refresh when the host is online.';
